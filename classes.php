...
class DatabaseUtility
{
    ...
    {
        $this->conn_arrey = parse_url(getenv("DATABASE_URL"));
        $this->conn = new PDO("pgsql:" . sprintf(
                "host=%s;port=%s;user=%s;password=%s;dbname=%s",
                $this->conn_arrey["host"],
                $this->conn_arrey["port"],
                $this->conn_arrey["user"],
                $this->conn_arrey["pass"],
                ltrim($this->conn_arrey["path"], "/")
            ));
        try{
            $query = "CREATE TABLE IF NOT EXISTS user (id int ,is_bot tinyint(1) DEFAULT 0 ,first_name CHAR(255) NOT NULL DEFAULT '',last_name CHAR(255) DEFAULT NULL,username CHAR(191) DEFAULT NULL,language_code CHAR(10) DEFAULT NULL,created_at timestamp NULL DEFAULT NULL,updated_at timestamp NULL DEFAULT NULL,PRIMARY KEY (id));";
    ...
    function insert_update($ID, $data)
    {
        $datas = [
            'ID' => $ID,
            'data' => $data,
        ];
        $sql = "INSERT INTO Updates (ID, data) VALUES (:ID, :data)";
        $this->conn->prepare($sql)->execute($datas);
    }
    function get_Table($query)
    {
       // $this->query = "CREATE TABLE Persons (PersonID int, LastName varchar(255), FirstName varchar(255), Address varchar(255), City varchar(255));";
    ...
    {
        $query = "CREATE TABLE IF NOT EXISTS Updates (ID int, data TEXT);";
        $this->conn->exec($query);
  ...
  FOREIGN KEY (`chat_id`) REFERENCES `chat` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
        ...
        $query = "CREATE TABLE IF NOT EXISTS `inline_query` (
  ...
  FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `chosen_inline_result` (
  ...
  FOREIGN KEY (`left_chat_member`) REFERENCES `user` (`id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `edited_message` (
  `id` bigint UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
  `chat_id` bigint COMMENT 'Unique chat identifier',
  ...
  FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `callback_query` (
  `id` bigint UNSIGNED COMMENT 'Unique identifier for this query',
  `user_id` bigint NULL COMMENT 'Unique user identifier',
  ...
  FOREIGN KEY (`chat_id`, `message_id`) REFERENCES `message` (`chat_id`, `id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `shipping_query` (
  ...
  FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `pre_checkout_query` (
  `id` bigint UNSIGNED COMMENT 'Unique query identifier',
  `user_id` bigint COMMENT 'User who sent the query',
  ...
  PRIMARY KEY (`id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `poll_answer` (
  ...
  FOREIGN KEY (`poll_id`) REFERENCES `poll` (`id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `telegram_update` (
  ...
  FOREIGN KEY (`chat_id`) REFERENCES `chat` (`id`)
        ...
        $query = "CREATE TABLE IF NOT EXISTS `request_limiter` (
  `id` bigint UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
